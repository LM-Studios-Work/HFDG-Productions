        ...
        <ol className="mt-16 grid grid-cols-1 gap-px overflow-hidden rounded-lg border-2 border-foreground bg-foreground md:grid-cols-3">
          {processSteps.map((step, i) => (
            ...
            >
                ...
                {i < processSteps.length - 1 && (
                  ...
                  >
                    {Array.from({ length: 9 }).map((_, t) => (
                      <span
                        key={t}
                        className={`w-px ${t === 4 ? "h-3.5 bg-accent" : "h-2 bg-foreground/30"}`}
                      />
                    ))}
                  </span>
                )}
              ...
              {/* Title + body: body uses grow so checklist always lands at the same
                  distance from the bottom of the card regardless of copy length */}
              ...
              {/* Checklist — anchored at the bottom, same vertical start each col */}
